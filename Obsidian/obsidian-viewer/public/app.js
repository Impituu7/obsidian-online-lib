...
async function loadNote(notePath) {
  ...
  try {
    const res = await fetch(`/api/notes/content?path=${encodeURIComponent(notePath)}`);
    ...
    link.addEventListener('mouseenter', (e) => {
      ...
      hoverTimeout = setTimeout(async () => {
        try {
          ...
          document.getElementById('preview-popup-body').textContent = 'Memuat pratinjau...';
          ...
          if (res.ok) {
            ...
            document.getElementById('preview-popup-body').textContent = 'Gagal memuat pratinjau catatan.';
          }
        } catch (err) {
          console.error(err);
        }
...
async function startQuiz(notePath) {
  ...
  try {
    const res = await fetch(`/api/notes/quiz?path=${encodeURIComponent(activeNotePath)}`);
    activeQuiz = await res.json();
    
    // Fetch outline details & tags for sidebar during quiz mode
    const resNote = await fetch(`/api/notes/content?path=${encodeURIComponent(activeNotePath)}`);
    let noteTitle = 'Materi';
    if (resNote.ok) {
      const noteData = await resNote.json();
      noteTitle = noteData.title;
      
      // Temporary populate outline nodes from content block
